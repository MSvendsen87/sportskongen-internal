...
function renderDeleteSupplierSection(parent, data, sb) {
  var section = createCollapsibleSection(
    "🗑️ Slett / deaktiver leverandør",
    "Slett leverandører som ikke er brukt. Leverandører som er brukt på produkter eller tillegg blir deaktivert i stedet.",
    false
  );

  var info = el("p", "Velg en leverandør du ønsker å fjerne. Hvis leverandøren er koblet til produkter eller tillegg, blir den deaktivert slik at historikken beholdes.");
  info.style.color = "#6b7280";
  section.body.appendChild(info);

  var suppliers = (data.suppliers || []).filter(function (s) {
    return s && s.id;
  });
  ...
  suppliers.forEach(function (s) {
    var label = s.name || "Ukjent leverandør";
    ...
    addOption(supplierSelect, s.id, label);
  });
  ...
  deleteBtn.onclick = function () {
    ...
    if (!supplierId) {
      alert("Velg en leverandør først.");
      return;
    }
    ...
    if (!selected) {
      alert("Fant ikke valgt leverandør.");
      return;
    }

    var confirmText = prompt(
      "Dette vil slette eller deaktivere leverandøren.\n\n" +
      "Leverandør: " + selected.name + "\n\n" +
      "Hvis leverandøren er brukt på produkter eller tillegg, blir den deaktivert i stedet for slettet.\n\n" +
      "Skriv SLETT LEVERANDØR for å bekrefte:"
    );

    if (confirmText !== "SLETT LEVERANDØR") {
      alert("Leverandøren ble ikke slettet. Du må skrive nøyaktig SLETT LEVERANDØR.");
      return;
    }
    ...
    sb.rpc("internal_delete_supplier", {
      ...
      if (row && row.action === "deleted") {
        alert("Leverandøren ble slettet.");
      } else if (row && row.action === "deactivated") {
        alert("Leverandøren ble deaktivert fordi den er brukt tidligere.");
      } else {
        alert("Ferdig.");
      }
  ...
  parent.appendChild(intro);
    ...
    renderDeleteSupplierSection(parent, data, sb);
